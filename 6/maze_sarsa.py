...
def run():
    ...
    for ep in range(max_episode):
        ...
        while(1): 
            ...
            a_agent.Policy[ob_m_next] = epsilon_greedy(a_agent.QValue[ob_m_next],\
                                            a_agent.epsilon,act_dim,round_swtich=False)
            act_next  = get_action(range(act_dim),a_agent.Policy[ob_m_next])

            a_agent.QValue[ob_m,act] += alpha*(reward+\
                                        a_agent.gamma*a_agent.QValue[ob_m_next,act_next] \
